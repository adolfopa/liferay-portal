...
public enum StoreArea {
	...
	DELETED("_deleted"), EVICTED("_evicted"), LIVE(StringPool.BLANK);
	...
	public static <T extends Throwable> void withStoreArea(
			StoreArea readStoreArea, UnsafeRunnable<T> unsafeRunnable)
		throws T {

		StoreArea oldStoreArea = _storeAreaThreadLocal.get();

		try {
			_storeAreaThreadLocal.set(readStoreArea);
	...
	private static final ThreadLocal<StoreArea> _storeAreaThreadLocal =
		new CentralizedThreadLocal<>(
			StoreArea.class.getName() + "._readStoreAreaThreadLocal",
			() -> LIVE);
