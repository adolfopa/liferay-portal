...
public abstract class BaseCommentResourceTestCase {
	...
	protected Response invokeGetStructuredContentCommentsPage( Long structuredContentId , Filter filter , Pagination pagination , Sort[] sorts ) throws Exception {
		RequestSpecification requestSpecification = _createRequestSpecification();

			return requestSpecification.when(
			).get(
				_resourceURL + "/structured-contents/{structured-content-id}/comments",
				structuredContentId , filter  , sorts
			);
