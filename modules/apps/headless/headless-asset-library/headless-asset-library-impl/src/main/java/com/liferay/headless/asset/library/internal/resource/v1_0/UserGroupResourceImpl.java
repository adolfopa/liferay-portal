...
public class UserGroupResourceImpl extends BaseUserGroupResourceImpl {
	...
	public Page<UserGroup> getAssetLibraryUserGroupsPage(
		...
		return _getUserGroupPage(
			assetLibraryId, keywords, search, filter, pagination, sorts);
	}
	...
	public UserGroup postAssetLibraryUserGroup(
		...
		return _toUserGroup(_userGroupService.getUserGroup(userGroup.getId()));
	}
	...
	private Page<UserGroup> _getUserGroupPage(
			long groupId, String keywords, String search, Filter filter,
			Pagination pagination, Sort[] sorts)
		throws Exception {
		...
		return SearchUtil.search(
			HashMapBuilder.put(
				...
				addAction(
					...
					"postAssetLibraryUserGroup", _groupModelResourcePermission)
			).put(
				"delete",
				addAction(
					ActionKeys.ASSIGN_MEMBERS, groupId,
			...
			filter, UserGroup.class.getName(), keywords, pagination,
			queryConfig -> {
			},
			searchContext -> {
