AUI.add(
	...
	function(A) {
		...
		var SELECTOR_SEARCH_NODES = 'ul li a';
		...
		var ControlPanel = A.Component.create(
			{
				ATTRS: {
					...
					sidebarHiddenAttr: {
						validator: Lang.isString,
						value: 'controlPanelSidebarHidden'
					},
					sidebarMinimizedAttr: {
						validator: Lang.isString,
						value: 'control-panel-sidebar-minimized'
					},
					togglerComponentId: {
						validator: Lang.isString
					}
				...
				prototype: {
					...
					_bindUI: function() {
						...
						var sidebarHiddenAttr = instance.get('sidebarHiddenAttr');

						if (panelHolder) {
							Liferay.set(sidebarHiddenAttr, body.hasClass(CSS_PANELS_MINIMIZED));

							Liferay.after(sidebarHiddenAttr + 'Change', instance._afterHiddenChange, instance);
							...
							if (body.hasClass(CSS_PANELS_MINIMIZED)) {
								...
								var controlPanelTools = instance._controlPanelTools;

								if (controlPanelTools) {
					...
					_createCancelButton: function() {
						...
						var searchNodes = A.all('#' + instance.get('panelContainerId') + ' ' + SELECTOR_SEARCH_NODES);
					...
					_createFocusManager: function() {
						...
						var addContentPanelContainer = A.one('#' + instance.get('panelContainerId'));
						...
						new Liferay.PanelSearch(
							{
								...
								nodeList: '#' + instance.get('panelContainerId'),
								nodeSelector: 'li',
								togglerId: instance.get('togglerComponentId')
							}
						);
					},
					...
					_panelHolderAccordionDisable: function() {
						...
						var component = Liferay.component(instance.get('togglerComponentId'));
					...
					_renderUI: function() {
						...
						if (searchPanelHolder) {
							var searchPanelInput = searchPanelHolder.one('#' + instance.get('searchPanelId'));
					...
					_toggleHidden: function(event) {
						...
						Liferay.set(sidebarHiddenAttr, !Liferay.get(sidebarHiddenAttr), EVENT_DATA_SIDEBAR);
					},
					...
					_uiSetHidden: function(newVal, persist) {
						...
						if (persist) {
							Liferay.Store(instance.get('sidebarMinimizedAttr'), newVal);
						}
